...
bert_scores_dir = f'{data_dir}/bert_score_text_all'
nli_scores_dir = f'{data_dir}/nli_score_text/'
...
def get_bert_scores(transcript_name, elasticsearch_transcript, verifiedClaims):
  ...
  print(bert_scores_f1.shape)
  vclaim_selected_indxs = get_vclaim_indxs_from_elasticsearch(elasticsearch_transcript)
  scores = np.zeros((len(bert_scores), 100))
  for i, (score, idx)  in enumerate(zip(bert_scores_f1, vclaim_selected_indxs)):
    scores[i] = score[idx]
  input = load_input(elasticsearch_transcript, scores, verifiedClaims)
  bert_scores_f1 = np.expand_dims(bert_scores_f1, axis=2)
  print('bert_scores_f1 scores shape: ', input.shape)
  return input
...
for i in range(len(input)):
  ...
  splits.append((zip(input_train, labels_train, vclaim_labels_train, vclaim_elasticsearch_train), zip(input_test, labels_test, vclaim_labels_test, vclaim_elasticsearch_test)))
...
for i, (train, test) in tqdm(enumerate(splits), total=len(splits)):
  for (input, label, vclaim_label, idxs) in test:
    predictions = input[:, :, 0]
    ...
    # print(rank)
    print(np.where(rank*label))

    vclaim_scores = input[:, :, 0]
...
with open('results-final.tsv', 'a') as f:
  f.write(f'single-top-{top_n}-{index}\n')
  f.write(out)
